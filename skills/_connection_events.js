...
module.exports = function(controller) {
    ...
    function conductOnboarding(bot, message) {
      ...
      bot.startConversation(message, function(err, convo) {
        ...
        convo.say({
          text: 'Hello! How are you doing? Do you know how effective are your meetings at work? ',
          quick_replies: [
            {
              title: 'No',
              payload: 'No',
    ...
    controller.hears(['no', "nope", "nah"], 'message_received', function(bot, message) {

      bot.startConversation(message, function(err, convo) {
        convo.say({
          text: 'Would you like to find out? Its easy, we can start by looking at your calander'
        });
        convo.addMessage({
          text: 'Click on the Calander link on the top above. You can take a peek in your calander...'
        });
        convo.addMessage({
          text: 'and we may find amazing staticstics on how we have been using our time.'
        });
      });
    });

    controller.hears(['yes', "yup", "yeah"], 'message_received', function(bot, message) {

      bot.startConversation(message, function(err, convo) {
        convo.say({
          text: 'Great! We can still look at finding ways to improve.'
        });
        convo.addMessage({
          text: 'Lets start with your calander. Do you know how many group meetings you have attended in on year?'
        });
        convo.addMessage({
          text: 'Lets start by clicking on the Calander link above'
        });
      });
    });
